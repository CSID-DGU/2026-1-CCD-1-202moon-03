...
function VideoCard({
  ...
  const hoverModeLabel = video.mode === 'rain' ? '집중호우' : '스피너';
  const showHoverSummary = !video.isPlaceholder;

  return (
    <article className={`relative ${isMenuOpen ? 'z-30' : 'z-0'}`}>
      <div className="relative overflow-hidden rounded-[20px] border border-[#DEE3EC] bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_36px_rgba(15,23,42,0.1)]">
                  ...
                  <div className="space-y-1 text-[15px] leading-[1.5]">
                    ...
                    {video.mode === 'rain' ? (
                      <>
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <span className="font-paperlogy text-white/88">최대 콤보</span>
                          <span className="font-paperlogy font-medium text-white">
                            {video.maxCombo ?? 0}
                          </span>
                        </div>
        ...
        <div className="absolute right-3 top-3 z-20">
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center rounded-full border bg-white/92 backdrop-blur-sm transition-colors ${
              isMenuOpen
                ? 'border-[#D8DEE8] text-[#15171C]'
                : 'border-transparent text-[#15171C] hover:bg-[#F3F5F9]'
            }`}
          >
            <DotsVerticalIcon />
          </button>
